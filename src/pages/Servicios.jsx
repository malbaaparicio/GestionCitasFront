  ...
  useEffect(() => {
   ...
   // --- MANEJADORES DEL MODAL ---
    ...
    const serviciosFiltrados = servicios.filter(serv => {
        ...
        const estadoActual = serv.estado || 'Activo';
    ...
    const serviciosPaginados = serviciosFiltrados.slice(indicePrimerRegistro, indiceUltimoRegistro);
      ...
      <h1 className="text-3xl font-bold mb-6 text-gray-800">Listado de Servicios</h1>
       ...
       {/* CABECERA: Título, Botón Nuevo y Toggle de Estado */}
            <div className="flex flex-row justify-between items-center bg-white p-4 rounded-lg shadow mb-6 w-full">
                ...
                {/* Izquierda: Botón Nuevo Servicio */}
                <button 
          ...
          {!cargando && serviciosFiltrados.length > 0 && (
              <div className="px-5 py-4 flex items-center justify-between border-t border-gray-200 bg-white">
                  <span className="text-sm text-gray-600">
                      Mostrando {indicePrimerRegistro + 1} a {Math.min(indiceUltimoRegistro, serviciosFiltrados.length)} de un total de {serviciosFiltrados.length} Servicios
                  </span>
